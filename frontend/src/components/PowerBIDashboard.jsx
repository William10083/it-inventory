...
const coverageColor = (pct) => {
    if (pct === null) return 'bg-slate-800 text-slate-500';
    if (pct >= 90) return 'bg-green-500/20 text-green-300';
    if (pct >= 70) return 'bg-yellow-500/20 text-yellow-300';
    if (pct >= 50) return 'bg-orange-500/20 text-orange-300';
    ...
    return (
        <div className="space-y-8 pb-16 animate-fade-in">
            ...
            <div className="flex items-center gap-3 bg-slate-800/50 p-4 rounded-xl border border-slate-700">
                ...
                <div>
                    <h2 className="text-2xl font-bold text-white">Power BI — Vista Ejecutiva</h2>
                    <p className="text-slate-400 text-sm">Métricas clave, tendencias y cobertura por sede</p>
                </div>
            </div>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                ...
                <div className="glass-card p-6">
                    <h3 className="text-lg font-bold text-white mb-1">Asignaciones por Mes</h3>
                    <p className="text-slate-400 text-xs mb-4">Actividad histórica de entregas de equipo</p>
                    <div className="h-64">
                ...
                <div className="glass-card p-6">
                    <h3 className="text-lg font-bold text-white mb-1">Distribución por Tipo</h3>
                    <p className="text-slate-400 text-xs mb-4">Composición del inventario activo</p>
            ...
            <div className="glass-card p-6">
                <h3 className="text-lg font-bold text-white mb-1">Heatmap de Cobertura por Sede</h3>
                    ...
                    <table className="w-full text-sm">
                        <thead>
                            <tr className="border-b border-slate-700">
                                <th className="text-left py-3 px-4 text-slate-400 font-medium min-w-[120px]">Sede</th>
                                <th className="text-center py-3 px-3 text-slate-400 font-medium text-xs">Empleados</th>
                                {EQUIPMENT_KEYS.map(k => (
                                    <th key={k} className="text-center py-3 px-3 text-slate-400 font-medium text-xs">{EQUIPMENT_LABELS[k]}</th>
                                ))}
                            </tr>
                        </thead>
                        <tbody>
                            {coverage_by_location.map((row) => (
                                <tr key={row.location} className="border-b border-slate-700/40 hover:bg-slate-800/30">
                                    <td className="py-3 px-4 font-medium text-white flex items-center gap-2">
                                        <MapPin className="w-3 h-3 text-slate-500" />{row.location}
                                    </td>
                                    <td className="py-3 px-3 text-center text-slate-300 text-sm">{row.total_employees}</td>
                                    {EQUIPMENT_KEYS.map(k => (
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                ...
                <div className="glass-card p-6">
                    <h3 className="text-lg font-bold text-white mb-1">Estado Global de Dispositivos</h3>
                    <p className="text-slate-400 text-xs mb-4">Conteo de equipos por estado (incluyendo vendidos y dados de baja)</p>
                    <div className="h-56">
                        <ResponsiveContainer width="100%" height="100%">
                            <BarChart data={status_breakdown} layout="vertical">
                                <CartesianGrid strokeDasharray="3 3" stroke="#374151" horizontal={false} />
                                <XAxis type="number" stroke="#9CA3AF" tick={{ fontSize: 11 }} />
                                <YAxis type="category" dataKey="status" stroke="#9CA3AF" tick={{ fontSize: 11 }} width={80} />
                                <Tooltip contentStyle={{ backgroundColor: '#1F2937', border: 'none', borderRadius: '8px', color: '#fff' }} />
                ...
                <div className="glass-card p-6">
                    <h3 className="text-lg font-bold text-white mb-1">Top Empleados por Equipos</h3>
                    <p className="text-slate-400 text-xs mb-4">Los 10 empleados con más dispositivos asignados actualmente</p>
                        ...
                        {top_equipped_employees.map((emp, i) => (
                            <div key={i} className="flex items-center gap-3 py-1.5 border-b border-slate-700/40 last:border-0">
                                ...
                                <div className="flex items-center gap-1">
                                    <span className="text-white font-bold text-sm">{emp.device_count}</span>
